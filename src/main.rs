...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            other => {
                ...
                let msg = tr.get("error.unknown_arg").cloned()
    ...
    let algorithm = match alg_opt {
        ...
        None => {
            let tr = load_translations(&lang);
            eprintln!("{}", tr.get("error.missing_alg").map(String::as_str)
    ...
    let variant = variant_opt.unwrap_or_else(|| 
        minimal_variant_for_algorithm(&algorithm).to_string());
    
    if variant.is_empty() {
        let tr = load_translations(&lang);
        let msg = tr.get("error.unknown_algorithm").cloned()
    ...
    let (mut pk_bytes, mut sk_bytes) = run_with_large_stack(
        move || generate_keys(&alg_clone, &var_clone), 
        &lang
    );

    // 키 저장 (비밀키는 0o600 권한으로 생성)
    save_keys(
        &pk_bytes, &sk_bytes, &pk_path, &sk_path, 
        &algorithm, &variant, pk_text, sk_text, &lang,
    );
...
fn usage_and_exit(prog: &str, lang: &str) {
    ...
    eprintln!("{}", tr.get("usage.supported").cloned()
        .unwrap_or_else(|| "지원되는 알고리즘: mlkem, hqc, mceliece, falcon, mldsa, sphincs+".to_string()));
    
    eprintln!("{}", tr.get("usage.req").cloned()
        .unwrap_or_else(|| "- 알고리즘은 필수로 명시해야 합니다. 배리언트는 알고리즘별 최소값이 기본입니다.".to_string()));
    ...
    eprintln!("{}", tr.get("usage.lang").cloned()
        .unwrap_or_else(|| "- [-lang]로 출력 로케일을 설정합니다. 기본값은 ko, en 지원.".to_string()));
    ...
    R: Send + 'static,
{
    ...
    let handle = builder.spawn(f).unwrap_or_else(|e| {
        let tr = load_translations(lang);
        let msg = tr.get("error.large_stack_create").cloned()
            .unwrap_or_else(|| "대용량 스택 스레드를 생성하는 도중 오류가 발생했습니다: {err}".to_string());
    ...
    match handle.join() {
        ...
        Err(_) => {
            let tr = load_translations(lang);
            let msg = tr.get("error.large_stack_panic").cloned()
                .unwrap_or_else(|| "대용량 스택 스레드에서 알 수 없는 패닉이 발생했습니다.".to_string());
            eprintln!("{}", msg);
            process::exit(1);
        }
    }
...
fn print_help_variants() {
    ...
    let sections = [
        ...
        Section {
            label: "mceliece",
            items: &[
                "348864", "348864", "460896", "460896", "348864f", "348864f", 
                "460896f", "460896f", "6688128", "6688128", "6960119", "6960119", 
                "8192128", "8192128", "6688128f", "6688128f", "6960119f", "6960119f", 
                "8192128f", "8192128f",
            ],
        },
        Section {
        ...
        Section {
            ...
            items: &[
                ...
                "shake_192f_simple", "shake_192s_simple", "shake_256f_simple", "shake_256s_simple",
            ],
        },
    ];
