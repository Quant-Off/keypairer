...
pub fn save_keys(
    ...
    let mut pk_file = match File::create(pk_path) {
        ...
        Err(e) => {
            ...
            let msg = tr.get("error.file.create_pk").cloned()
                .unwrap_or_else(|| "공개키 파일을 생성하는 도중 오류가 발생했습니다: {err}".to_string());
    ...
    if pk_text {
        ...
        if let Err(e) = pk_file.write_all(pem.as_bytes()) {
            let tr = super::i18n::load_translations(lang);
            let msg = tr.get("error.file.write_pk").cloned()
    ...
    let mut sk_file = match {
        ...
        Err(e) => {
            let tr = super::i18n::load_translations(lang);
            let msg = tr.get("error.file.create_sk").cloned()
                .unwrap_or_else(|| "비밀키 파일을 생성하는 도중 오류가 발생했습니다: {err}".to_string());
            eprintln!("{}", msg.replace("{err}", &e.to_string()));
            process::exit(1);
        }
    };
    ...
    if sk_text {
        ...
        if let Err(e) = sk_file.write_all(pem.as_bytes()) {
            let tr = super::i18n::load_translations(lang);
            let msg = tr.get("error.file.write_sk").cloned()
